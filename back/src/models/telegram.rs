...
impl Telegram {
    ...
    pub async fn get(pool: &SqlitePool) -> Result<Telegram, Error> {
        debug!("get_telegram");
        let active_str = Param::get(pool, "telegram_active").await?;
        let active = active_str == "TRUE";
        let token = Param::get(pool, "telegram_token").await?;
        let chat_id = Param::get(pool, "telegram_chat_id").await?;
        let thread_id = Param::get(pool, "telegram_thread_id").await?;
    ...
    pub async fn set(pool: &SqlitePool, telegram: &Telegram) -> Result<Telegram, Error> {
        ...
        Param::set(
            pool,
            "telegram_active",
            &telegram.active.to_string().to_uppercase(),
        )
        .await?;
...
mod test {
    ...
    async fn telegram() {
        tracing_subscriber::registry()
            ...
            .init();
        dotenv().ok();
        let token = env::var("TELEGRAM_TOKEN").expect("Cant get token");
        ...
        let response = telegram
            .send_audio(filename, audio, "Esto es una prueba")
            .await;
        println!("{:?}", response);
