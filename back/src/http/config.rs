...
use crate::models::{ApiResponse, AppState, Data, Feed, Telegram, Twitter};
...
pub async fn read_twitter(State(app_state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    match Twitter::set(&app_state.pool, &twitter).await {
        Ok(twitter) => {
            ...
            ApiResponse::new(
                StatusCode::OK,
                "Twitter saved",
                Data::One(serde_json::to_value(twitter).unwrap()),
            )
        }
        Err(e) => {
...
pub async fn read_telegram(State(app_state): State<Arc<AppState>>) -> impl IntoResponse {
    match Telegram::get(&app_state.pool).await {
        Ok(telegram) => {
            ...
            ApiResponse::new(
                StatusCode::OK,
                "Telegram read",
                Data::One(serde_json::to_value(telegram).unwrap()),
            )
        }
        Err(e) => {
            error!("Error reading feed: {:?}", e);
