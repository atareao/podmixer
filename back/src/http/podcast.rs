...
use crate::models::{ApiResponse, AppState, CompletePodcast, Data, Feed, Id, NewPodcast, Podcast};
...
pub async fn create(
    ...
    match Podcast::create(
        &app_state.pool,
        &podcast.name,
        &podcast.url,
        podcast.active,
        &podcast.last_pub_date,
    )
    .await
    {
        Ok(podcast) => {
            ...
            ApiResponse::new(
                StatusCode::CREATED,
                "Podcast created",
                Data::One(serde_json::to_value(podcast).unwrap()),
            )
        }
        Err(e) => {
    ...
    match Podcast::get(&app_state.pool).await {
        Ok(podcasts) => {
            ...
            ApiResponse::new(
                StatusCode::OK,
                "Podcasts",
                Data::One(serde_json::to_value(podcasts).unwrap()),
            )
        }
        Err(e) => {
...
pub async fn delete(State(app_state): State<Arc<AppState>>, id: Query<Id>) -> impl IntoResponse {
    debug!("Podcast: {:?}", id);
...
pub async fn regenerate_feed(State(app_state): State<Arc<AppState>>) -> impl IntoResponse {
