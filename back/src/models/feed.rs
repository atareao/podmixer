...
use rss::{
    extension::itunes::{ITunesCategoryBuilder, ITunesChannelExtensionBuilder, ITunesOwnerBuilder},
    CategoryBuilder, ChannelBuilder, ImageBuilder, Item,
};
...
impl Feed {
    ...
    pub async fn set(pool: &SqlitePool, feed: &Feed) -> Result<Feed, Error> {
        ...
        Param::set(
            pool,
            "feed_explicit",
            &feed.explicit.to_string().to_uppercase(),
        )
        .await?;
    ...
    pub fn rss(&self, episodes: Vec<Item>) -> Result<String, Error> {
        let image = ImageBuilder::default()
            ...
            .link(&self.link)
            .build();
        let category = CategoryBuilder::default().name(&self.category).build();
        ...
        let itunes = ITunesChannelExtensionBuilder::default()
            ...
            .explicit(Some(if self.explicit {
                "yes".to_string()
            } else {
                "no".to_string()
            }))
        ...
        namespaces.insert(
            "wfw".to_string(),
            "http://wellformedweb.org/CommentAPI/".to_string(),
        );
        namespaces.insert(
            "itunes".to_string(),
        ...
        namespaces.insert(
            "feedpress".to_string(),
            "https://feed.press/xmlns".to_string(),
        );
        namespaces.insert(
            "podcast".to_string(),
        ...
        namespaces.insert(
            "googleplay".to_string(),
            "http://www.google.com/schemas/play-podcasts/1.0".to_string(),
        );
        namespaces.insert(
            "podcast".to_string(),
            "https://podcastindex.org/namespace/1.0".to_string(),
